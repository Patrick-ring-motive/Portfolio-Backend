...
const createList = async function createList(req, res) {
  ...
  const {
    userID
  } = req.user; // Assuming userID is available in req.user if authenticated
...
const deleteList = async function deleteList(req, res) {
  ...
  const {
    userID
  } = req.user; // Assuming the userID is available in req.user if authenticated
  ...
  try {
    ...
    res.status(500).json({
      message: 'Error deleting list and items'
    });
  }
};
